...
= 静态规划
静态规划讨论的是在不考虑系统随时间动态演变的情况下, 如何寻找最优决策.

给定一个标量性能指标 $L(u)$, 寻找控制变量 $u$ 的值, 使 $L$ 达到极小

= 动态规划 (DP)
动态规划是解决多阶段决策过程优化的基础理论, 由贝尔曼 (Bellman) 提出.
== 最优控制问题
...
$
  x_(k + 1) = A x_k + B u_k
$
其中 $x_k$ 是状态, $u_k$ 是控制输入, $A$ 和 $B$ 分别是系统矩阵和输入矩阵.

*性能指标*
$
  J = sum_(k = 0)^(N - 1) (x_k^T Q x_k + u_k^T R u_k) + x_N^T H x_N
$
其中
- $x_k^T Q x_k$ 是状态成本, $Q$ 是半正定矩阵. 权重越大, 要求状态在演化过程中尽早接近原点.
- $u_k^T R u_k$ 是控制成本, $R$ 是 *正定矩阵*. 权重越大, 表示对能量损耗的约束越严.
- $x_N^T H x_N$ 是终端状态成本,  $H$ 是半正定矩阵. 权重越大, 要求终止时刻的状态越接近原点.

== DP 求解
我们使用离散 DP 来求解一步的 LQR 问题. 令 $V(x, k)$ 表示从时刻 $k$ 开始, 状态为 $x$ 时的最优价值函数. 考虑 $H != 0$, 所以我们有 $V(x, N) = x^T H x$ .

现在考虑 $k = N - 1$ 的情况.
$
  V(x, N - 1) = min_u {x^T Q x + u^T R u + V(A x + B u, N)} \
  = min_u {x^T Q x + u^T R u + \(A x + B u)^T H (A x + B u)} \
  = min_u {u^T (R + B^T H B) u + 2 x^T A^T H B u + x^T (Q + A^T H A) x}
$
取
$
  u = - (2 x^T a^T H) / (2 (R + B^T H B)) = - (R + B^T H B)^(-1) B^T H A x
$

= 自适应动态规划 (ADP)
== 传统 DP 的局限性
- *维数灾难*: 随着状态变量数量的增加, 状态空间呈指数级增长, 导致计算和存储需求急剧上升.
- *无限域最优控制问题*: 由于没有终止时刻, 无法使用 "倒推法" 求解 Bellman 方程.

== ADP 的基本思想
ADP 通过结合动态规划与函数逼近技术, 旨在克服传统动态规划的局限性. 它通过以下几个关键思想实现:
1. *函数逼近*: 使用神经网络, 树模型等函数逼近器来近似最优价值函数或策略函数, 从而避免显式地存储整个状态空间的值.
2. *在线学习*: 通过与环境的交互, 实时更新价值函数或策略, 使得算法能够适应动态变化的环境.


ADP 系统通常由三个核心神经网络组成, 模拟智能体的学习过程:
1. *模型网络* (Model Network): 负责学习和近似环境的动态模型, 即状态转移函数 $f(x, u)$ . 通过观察状态转移数据, 模型网络能够预测给定当前状态和动作下的下一状态.
2. *评判网络* (Critic Network): 负责估计状态的价值函数 $V(x)$ 或动作价值函数 $Q(x, u)$ . 评判网络通过最小化时序差分误差 (Temporal Difference Error) 来学习, 以提供对当前策略的评估.
3. *执行网络* (Actor Network): 负责生成控制策略 $u = pi(x)$ . 执行网络通过最大化评判网络提供的价值估计来调整策略, 以实现最优控制.

== HDP
=== 原始系统模型
我们有一个抽象的动态系统, 其状态转移满足
$
  x_(k + 1) = f(x_k, u_k)
$
同时我们有一个已知的单步效用函数 $l(x_k, u_k)$, 并由此定义我们的优化目标:
$
  J(x_k) = sum_(i=0)^(oo) l(x_(k + i), u_(k + i))
$
它代表了从时刻 $k$ 开始, 未来所有时刻的累计效用 (Cost). 我们希望通过选择合适的控制策略 $u_k$ 来最小化 $J(x_k)$ .

=== 通过 NN 来近似系统
由于状态转移函数的具体形式可能很复杂, 所以我们假设 $f$ 就是一个纯黑盒模型. 转而去尝试用一个模型网络 (MN) 拟合它:
$
  hat(x)_(k + 1) approx "MN"(x_k, u_k; W_m)
$
这里 $W_m$ 是模型网络的参数.

同样地, 我们使用评判网络 (CN) 来近似最优价值函数:
$
  hat(J)(x_k) approx "CN"(x_k; W_c)
$
和执行网络 (AN) 来近似最优策略:
$
  hat(u)_k approx "AN"(x_k; W_a)
$

那么 NN 近似模型从 $x(k) -> J(x_k)$ 的计算过程可以表示为:
$
  x_k xarrow("AN") u_k \ (x_k, u_k) xarrow("MN") x_(k + 1) \ x_(k + 1) xarrow("CN") J(x_(k + 1)) \ J(x_k) = l(x_k, u_k) + J(x_(k + 1)) \
$

=== NN 的优化过程
有了模型定义, 我们还需要 Loss 函数来辅助模型进行学习.

*a) MN 的 Loss 函数*: \
由于 MN 的任务就是拟合 $f$, 那么我们就用真实观察到的下一个状态作为监督信号:
$
  e_"MN" = x_(k + 1) - hat(x)_(k + 1) , quad hat(x)_(k + 1) = "MN"(x_k, u_k; W_m) \
  cal(L)_"MN" = norm(e_"MN")_2^2
$
这里 $x_(k + 1)$ 就是真实系统输入 $(x_k, u_k)$ 后观察到的下一个状态.

*b) CN 的 Loss 函数*: \
考虑 Bellman 方程, 对于理想的 $J(x_k)$ 应该有:
$
  J(x_k) = l(x_k, u_k) + J(f(x_k, u_k))
$
但是实际上的模型预测出来的 $hat(J) != J$, 所以上述等式并不成立, 我们令 Loss 函数为两边的误差:
$
  e_"CN" = hat(J) (x_(k + 1)) - [J(x_k) - l(x_k, u_k)] \
  cal(L)_"CN" = norm(e_"CN")_2^2
$
这里 $J(x_k)$ 是上一步 CN 的输出, 在这里被当做一个常数 (i.e. 不参与梯度计算).

*c) AN 的 Loss 函数*: \
AN 的目标是让 $J(x_k)$ 接近我们期望的最终目标代价, 即
$
  e_"AN" = J(x_k) - U_D
$
这里 $U_D$ 是一个预设的目标代价. 在大多数控制问题中, 我们希望代价最终归零 (越小越好), 所以通常取 $U_D = 0$. 因此 AN 的 Loss 函数为:
$
  cal(L)_"AN" = 1/2 hat(J)(x_k)^2
$

可以使用 PyTorch 伪代码来总结一下整个优化过程. 给定一组真实采样的 $(x_k, u_k, x_(k+1))$, 通过如下步骤学习:
```python
def train_step(x_k, u_k, x_next_real):
    # 1. Optimize Model Network (MN)
    x_next_pred = MN(x_k, u_k)
    loss_m = F.mse_loss(x_next_pred, x_next_real)
    opt_m.zero_grad(); loss_m.backward(); opt_m.step()

    # 2. Optimize Critic Network (CN)
    target_c = CN(x_k).detach() - l(x_k, u_k)
    pred_c = CN(x_next_real)
    loss_c = F.mse_loss(pred_c, target_c)
    opt_c.zero_grad(); loss_c.backward(); opt_c.step()

    # 3. Optimize Actor Network (AN)
    u_k_new = AN(x_k)
    x_next_sim = MN(x_k, u_k_new)
    j_next = CN(x_next_sim)
    j_now = j_next + l(x_k, u_k_new)
    loss_a = j_now # Preventing gradient vanishing
    opt_a.zero_grad(); loss_a.backward(); opt_a.step()
```
=== 梯度更新的手动计算
如果我们想手动计算梯度下降, 先要定义 MN, CN, AN 的网络结构. 我们假设它们都是带一个隐藏层的 MLP, 激活函数使用 $tanh$, 且不使用偏置项:
$
  "MN": RR^n times RR^m -> RR^n \
  (x_k , u_k) mapsto W_"m2" tanh(W_"m1" (x_k ; u_k))
$
这里 $W_"m1" in RR^(h times (n + m))$, $W_"m2" in RR^(n times h)$ , $tanh$ 是逐元素计算的.
$
  "CN": RR^n -> RR \
  x_k mapsto W_"c2" tanh(W_"c1" x_k)
$
其中 $W_"c1" in RR^(h times n)$ , $W_"c2" in RR^(1 times h)$.
$
  "AN": RR^n -> RR^m \
  x_k mapsto W_"a2" tanh(W_"a1" x_k)
$
其中 $W_"a1" in RR^(h times n)$ , $W_"a2" in RR^(m times h)$.

现在可以开始计算梯度:
#showybox(
  title: [*Tips: $tanh(x)$ 的导数*],
  frame: frameSettings,
)[
  在标量情形下, 有
  $
    y(x) = tanh(x) = (e^x - e^(-x)) / (e^x + e^(-x)) \
    dv(y, x) = ((e^x + e^(-x))^2 - (e^x - e^(-x))^2) / (e^x + e^(-x))^2 = 1 - y^2
  $
  推广到 $x, y in RR^n$, 则有
  $
    pdv(y, x) = "diag"(1 - y_1^2, ..., 1 - y_n^2) = "diag"(1- tanh^2(x))
  $
]
...
令
$
  E_m = 1/2 e_m^T e_m , quad e_m = x_(k + 1) - hat(x)_(k + 1)
$
这里直接使用链式法则处理张量的维度会很麻烦, 我们直接使用全微分:
$
  dif E_m = e_m^T dif e_m = - e_m^T dif hat(x)_(k + 1) \
$
设 $p_m = tanh(W_"m1" (x_k ; u_k))$, 则 $dif hat(x)_(k + 1) = dif W_"m2" p_m$, 从而
$
  dif E_m = -e_m^T dif W_"m2" p_m = - tr(-e_m^T dif W_"m2" p_m) = - tr(p_m e_m^T dif W_"m2") \
$
于是
$
  markrect(pdv(E_m, W_"m2") = -e_m p_m^T)
$
接下来计算 $W_"m1"$ 的梯度. 继续使用全微分:
$
  dif hat(x)_(k+1) = W_"m2" dif p_m = W_"m2" (1 - "diag"(p_m^2)) dif (W_"m1" (x_k ; u_k)) \
$
设 $z_k = (x_k ; u_k)$, 则有
$
  dif hat(x)_(k+1) & = W_"m2" (1 - "diag"(p_m^2)) (dif W_"m1") z_k \
       ==> dif E_m & = - e_m^T W_"m2" (1 - "diag"(p_m^2)) (dif W_"m1") z_k \
                   & = - tr[z_k e_m^T W_"m2" (1 - "diag"(p_m^2)) dif W_"m1"] \
$ <dif-e-m>
于是
$
  markrect(pdv(E_m, W_"m1") = - (1 - "diag"(p_m^2)) W_"m2"^T e_m z_k^T)
$

*b) CN 的梯度计算*: \
我们设 $J_"tgt" = J(x_k) - l(x_k, u_k)$, 从而有 $e_c = hat(J)(x_(k + 1)) - J_"tgt"$. 类似于 MN 的计算, 令 $p_c = tanh(W_"c1" x_(k + 1))$, 则
$
  dif E_c = e_c^T dif e_c = e_c^T dif hat(J)(x_(k + 1)) = e_c^T dif W_"c2" p_c \
$
于是
$
  dif E_c = tr (p_c e_c^T dif W_"c2") ==> markrect(pdv(E_c, W_"c2") = e_c p_c^T)
$
接下来计算 $W_"c1"$ 的梯度:
$
  dif E_c & = e^T_c W_"c2" dif p_c = e^T_c W_"c2" (1 - "diag"(p_c^2)) dif (W_"c1" x_(k + 1)) \
          & = tr[x_(k + 1) e_c^T W_"c2" (1 - "diag"(p_c^2)) dif W_"c1"] \
$<dif-e-c>
于是
$
  markrect(pdv(E_c, W_"c1") = (1 - "diag"(p_c^2)) W_"c2"^T e_c x_(k + 1)^T)
$

*c) AN 的梯度计算*: \
因为 AN 的计算图比较长, 我们先整理一下各个中间变量:
$
  x_k xarrow("AN"(dot.c)) u_k xarrow("MN"(x_k, dot.c)) x_(k + 1) xarrow("CN"(dot.c)) hat(J)(x_(k + 1)) xarrow(+l(x_k, u_k)) J(x_k) --> E_a
$
后面就是简单但是 dirty 的计算, 我们从右向左开始:
1. $E_a -> J(x_k)$:
$
  dif E_a = e_c dif e_c = e_c dif J(x_k)
$
2. $J(x_k) -> hat(J)(x_(k + 1))$:
$
  dif J(x_k) = dif hat(J)(x_(k + 1))
$
3. $hat(J)(x_(k + 1)) -> x_(k + 1)$, 复用 @dif-e-c:
$
  dif hat(J)(x_(k + 1)) = W_"c2" (1 - "diag"(p_c^2)) dif (W_"c1" x_(k + 1)) \
  = W_"c2" (1 - "diag"(p_c^2)) W_"c1" dif x_(k + 1)
$
4. $x_(k + 1) -> u_k$, 复用 @dif-e-m:
$
  dif x_(k + 1) = W_"m2" (1 - "diag"(p_m^2)) dif (W_"m1" (x_k ; u_k))
$
为了方便起见, 我们把 $W_"m1"$ 切片为 $W_"m1" = [W_"m1x" , W_"m1u"]$, 其中 $W_"m1x" in RR^(h times n)$ , $W_"m1u" in RR^(h times m)$ . 则
$
  dif x_(k + 1) = W_"m2" (1 - "diag"(p_m^2)) W_"m1u" dif u_k
$
5. $u_k -> W_"a2"$, 令 $p_a = tanh(W_"a1" x_k)$ , 则
$
  dif u_k = dif W_"a2" p_a
$
把之前算出来的一坨都代入, 令 $cal(T)(p) := 1 - "diag"(p^2)$ , 则有
$
  dif E_a = e_c W_"c2" cal(T)(p_c) W_"c1" W_"m2" cal(T)(p_m) W_"m1u" dif W_"a2" p_a \
  = tr[p_a e_c W_"c2" cal(T)(p_c) W_"c1" W_"m2" cal(T)(p_m) W_"m1u" dif W_"a2"] \
$ <dif-e-a2>
于是
$
  markrect(pdv(E_a, W_"a2") = [p_a e_c W_"c2" cal(T)(p_c) W_"c1" W_"m2" cal(T)(p_m) W_"m1u"]^T)
$
6. $u_k -> W_"a1"$: 利用 @dif-e-a2 的结果:
$
  dif E_a = e_c W_"c2" cal(T)(p_c) W_"c1" W_"m2" cal(T)(p_m) W_"m1u" W_"a2" dif p_a \
$
而 $dif p_a = cal(T)(p_a) dif (W_"a1" x_k) = cal(T)(p_a) dif W_"a1" x_k$, 因此
$
  dif E_a = tr[x_k e_c W_"c2" cal(T)(p_c) W_"c1" W_"m2" cal(T)(p_m) W_"m1u" W_"a2" cal(T)(p_a) dif W_"a1"] \
$
于是
$
  markrect(pdv(E_a, W_"a1") = [x_k e_c W_"c2" cal(T)(p_c) W_"c1" W_"m2" cal(T)(p_m) W_"m1u" W_"a2" cal(T)(p_a)]^T)
$
