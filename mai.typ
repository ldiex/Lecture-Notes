...
由此可以写出对数似然函数
$ ell \( bold(w) \) & = sum_(i = 1)^N log p \( y_i divides bold(x)_i \) = - sum_(i = 1)^N log \( 1 + e^(- y_i bold(w)^T bold(x)_i) \)\ $ 在此基础上加上正则项
$
...
$
  nabla^2 f(x) =
  mat(
    delim: "[", (diff^2 f(x))/(diff x_1^2), (diff^2 f(x))/(diff x_1 diff x_2), (diff^2 f(x))/(diff x_1 diff x_3), dots.c, (diff^2 f(x))/(diff x_1 diff x_n);
...
=== Lagrangian
对于约束优化问题, 我们定义其 *Lagrangian (Lagrangian Function)* 为
$
...
$ min_(x in cal(D)) & quad f_0(x) \
           "s.t." & quad c_i (x) lt.eq 0, quad i = 1, dots.h, m \
                  & quad A x = b $ 其中 $f_0$ 是适当的凸函数, $cal(D)$ 是 $x$ 的自然域, $c_i$ 是凸函数, $A in RR^(p times n)$, $b in RR^p$.
=== Slater 约束品性与强对偶原理
...
#showybox(
  ...
  我们之前提到, Lagrange 对偶函数通过给违反约束 ($c_i (x) > 0$) 的点施加惩罚 (通过 $lambda_i > 0$ 控制) 来帮助我们找到最优解. 如果存在一个严格可行点 (即满足 $c_i (x) < 0$ 的点), 那么对于某些 $lambda$, Lagrangian可以在该点取到一个合理的值, 而让对偶函数接近原始问题的最优值.

  但是如果所有可行点都满足某些 $c_i (x) = 0$, Lagrangian可能被迫在这些边界点取值, 这限制了我们通过调整 $lambda$ 来优化对偶函数的能力. 这就像是在一个狭窄的走廊里寻找最优解, 你没有足够的空间来 "呼吸" 和调整你的策略.
  ...
  该问题的可行域实际上是 $x = 0, y > 0$ 的集合, 因为只有在 $x = 0$ 时, $x^2/y <= 0$ 才成立, 所以原问题的最优值为 $p^* = 1$. 现在我们考虑其 Lagrangian:
  $
...
== 一般约束优化问题的最优性理论
=== 切锥
给定可行域 $cal(X)$ 以及 $x in cal(X)$, 若存在序列 ${z_k}_(k = 1)^oo subset cal(X)$, $lim_(k -> oo) z_k = x$, 以及一列正数 ${t_k}_(k = 1)^oo$ 满足 $t_k -> 0$, 使得
$
  lim_(k -> oo) (z_k - x)/t_k = d
$
...
假设可行点 $x^*$ 是上述问题的一个局部极小点, 如果 $f(x)$ 和 $c_i (x), i in cal(I) union cal(E)$ 在点 $x^*$ 处均可微, 则有
$
  d^T nabla f_0 (x^*) >= 0, quad forall d in T_(cal(X)) (x^*)
$
等价于
$
  T_(cal(X)) (x^*) inter { d in RR^n | d^T nabla f_0 (x^*) < 0 } = emptyset
$

=== 线性化可行锥
对于可行点 $x in cal(X)$, 定义该点的 *积极集* $cal(A)(x) = cal(E) union { i in cal(I) | c_i (x) = 0 }$. 点 $x$ 处的 *线性化可行锥 (Linearized Feasible Cone)* 定义为
$
  cal(F)(x) = { d in RR^n | & d^T nabla c_i (x) = 0 quad forall i in cal(E); \
                            & d^T nabla c_i (x) <= 0 quad forall i in cal(A)(x) inter cal(I) }
$
...
#showybox(
  ...
  几何最优性条件的直观解释是: 如果 $x^*$ 是一个局部极小点, 那么在切锥中的任何方向 $d$ 上, 目标函数 $f_0$ 的梯度 $nabla f_0 (x^*)$ 都不能指向下降的方向. 换句话说, 沿着任何可行方向移动, 目标函数的值都不会减小.

  线性化可行锥 $cal(F)(x)$ 则是对切锥的一种近似. 它通过线性化约束条件来定义可行方向, 忽略了约束的非线性部分. 我们不再关心可行域 $cal(X)$ 真实的, 可能是弯曲的边界, 我们只看 $x$ 点当前当前接触到的 "墙", 也就是积极集 $cal(A)(x)$ 中的约束.

  $cal(F)(x)$ 就是由这些线性 "墙" 所围成的可行方向集合:
...
$
  cal(F)(x^*) = { d | & d^T nabla c_i (x^*) = 0, quad forall i in cal(E); \
                      & d^T nabla c_i (x^*) <= 0, quad forall i in cal(A)(x^*) inter cal(I) }
$
Farkas 引理表明
$
...
== 线搜索准则
=== 基本思想
梯度下降算法的核心思想是沿着目标函数梯度的反方向移动, 以期望降低函数值. 具体来说, 给定当前点 $x_k$, 我们计算梯度 $nabla f(x_k)$, 并选择一个步长 $alpha_k > 0$, 然后更新点:
$
  x_(k+1) = x_k - alpha_k nabla f(x_k)
$
我们可以简化为 $x_(k+1) = x_k + alpha_k d_k$, 其中 $d_k$ 是下降方向. 这里就引入两个关键问题:
1. *如何选择下降方向 $d_k$?* 最常见的选择是 $d_k = -nabla f(x_k)$, 这个是最好的选择吗?
2. *如何选择步长 $alpha_k$?* 步长的选择对算法的收敛性和效率有很大影响.

=== 精确线搜索
精确线搜索假设我们可以精确地找到使得目标函数在下降方向上取得最小值的步长. 具体来说, 给定当前点 $x_k$ 和下降方向 $d_k$, 我们计算
$
  alpha_k = arg min_(alpha > 0) f(x_k + alpha d_k)
$
这种方法的优点是每一步都能最大限度地降低函数值, 但缺点是计算代价高, 因为需要在每一步都解决一个一维优化问题.

=== 非精确线搜索
非精确线搜索通过设定一些条件来选择步长, 以避免每次都进行精确优化. 常见有 *Wolfe 准则*:
1. *充分下降条件 (Armijo 条件)*: 步长不能太大, 以确保函数值有足够的下降:
$
  f(x_k + alpha_k d_k) <= f(x_k) + c_1 alpha_k nabla f(x_k)^T d_k
$
2. *曲率条件*: 步长不能太小, 以确保我们在下降方向上有足够的进展:
$
...
== 线搜索一般收敛性分析
=== Zoutendijk 条件
设函数 $f: RR^n -> RR$ 在 $RR^n$ 上连续可微, 且其梯度 $nabla f$ 是 Lipschitz 连续的, 即存在常数 $L > 0$, 使得对于任意 $x, y in RR^n$, 有
$
  norm(nabla f(x) - nabla f(y)) <= L norm(x - y)
$
假设算法生成的点列 ${x_k}_(k = 0)^oo$ 满足 Wolfe 准则, 则有
$
  sum_(k = 0)^oo [ (nabla f(x_k)^T d_k) / norm(d_k) ]^2 < +oo
$
这个不等式也被称为 *Zoutendijk 定理*.

这个条件也可以被写成
$
  sum_(k = 0)^oo cos^2(theta_k) norm(nabla f(x_k))^2 < +oo
$
这里 $cos(theta_k) = (-nabla f(x_k)^T d_k) / (norm(nabla f(x_k)) norm(d_k))$ 是梯度和下降方向之间的夹角余弦.

=== 收敛性分析

通过 Zoutendijk 条件, 我们可以分析梯度下降算法的收敛性. 如果对任意的 $k$, 都存在常数 $gamma > 0$ 满足
$
  theta_k < pi/2 - gamma
$
则在 Zoutendijk 条件成立的前提下, 有
$
  lim_(k -> oo) norm(nabla f(x_k)) = 0
$
这是因为如果梯度的范数不趋近于零, 则 $cos^2(theta_k) norm(nabla f(x_k))^2$ 有一个正的下界, 导致级数发散, 与 Zoutendijk 条件矛盾.

特别地, 我们考虑梯度下降法, 即 $d_k = -nabla f(x_k)$, 则 $theta_k = 0$, 满足上述条件, 因此梯度下降法在 Wolfe 准则下收敛到一个临界点.

== 梯度下降法的收敛性
=== 凸函数的情况
现在我们假设 $f(x)$:
1. 是凸函数;
2. 是 $L$-光滑的, 即其梯度满足 Lipschitz 条件 $norm(nabla f(x) - nabla f(y)) <= L norm(x - y)$;

$L$-光滑性保证了函数可以通过一个二次函数来进行全局上限估计:
$
  f(y) <= f(x) + nabla f(x)^T (y - x) + (L/2) norm(y - x)^2, quad forall x, y in RR^n
$
如果我们采用固定的步长 $alpha <= 1/L$, 则函数的收敛速率为 $O(1/k)$:
$
  f(x_k) - f(x^*) <= (norm(x_0 - x^*)^2) / (2 alpha k)
$
这是 *次线性收敛*, 意味着误差大小随着迭代次数的增加以 $1/k$ 的速度减小.
=== 强凸函数的情况
如果 $f(x)$ 进一步是 *强凸的*, 即存在常数 $m > 0$, 使得对于任意 $x, y in RR^n$, 有
$
  f(y) >= f(x) + nabla f(x)^T (y - x) + (m/2) norm(y - x)^2
$
则采用固定步长如 $alpha < 2/(m + L)$ 时, 函数的收敛速率为 $Q$-线性收敛:
$
  f(x_k) - f(x^*) <= (1 - alpha (2m L)/(m + L)) norm(x_(k-1) - x^*)^2
$
这里的收敛因子 $c = 1 - alpha (2m L)/(m + L) < 1$, 其大小和 条件数 $kappa = L/m$ 有关. 条件数越大, 收敛因子越接近 1, 收敛速度越慢.

这是 *线性收敛*, 意味着误差大小随着迭代次数的增加以指数速度减小, 每次都缩小一个固定的比例.

=== 总结
对于梯度下降法, 在不同的函数性质下, 收敛性表现如下:
#figure(
  table(
    ...
    align: horizon,
    [*函数性质*], [*核心假设*], [*步长选择*], [*收敛速率*],
    [一般可微], [$L$-光滑], [Wolfe 准则], [收敛但不保证速率],
    [凸函数], [$L$-光滑], [固定步长 $alpha <= 1/L$], [$O(1 / epsilon)$ 次线性收敛],
    [强凸函数], [$L$-光滑 + 强凸], [固定步长 $alpha < 2/(m + L)$], [$O(log (1 / epsilon))$ 线性收敛],
  ),
)

== Barzilai-Borwein 方法
Barzilai-Borwein (BB) 方法是一种改进的梯度下降法, 通过选择特殊的步长来加速收敛. 其核心思想是利用前两次迭代的信息来估计目标函数的局部曲率, 从而选择一个更合适的步长.

== 光滑法
光滑法 (Smoothing Method) 是一种用于求解非光滑优化问题的技术. 其基本思想是通过引入一个平滑参数, 将非光滑函数近似为一系列光滑函数, 然后在这些光滑函数上应用传统的优化方法 (如梯度下降法).

比如对于 LASSO 问题, 我们可以改写不可微的 $norm(x)_1$ 项为一个光滑近似:
$
  l_delta (x) = cases(
    1/ (2 delta) x^2 \, & quad |x| <= delta,
    |x| - delta / 2 \, & quad |x| > delta
  )
$
这个近似是 Lipschitz 连续可微的, 并且当 $delta -> 0$ 时, $l_delta (x) -> |x|$. 通过逐渐减小 $delta$, 我们可以在一系列光滑问题上迭代求解, 最终逼近原始的非光滑问题的解.
= 次梯度算法
== 非光滑的情况
我们之前熟悉的梯度下降法要求目标函数的每一点都是可微的, 也就是说每一点都要有 *唯一* 的梯度.

对于非光滑的凸函数, 我们使用次梯度 (@sub-gradient) 来代替梯度. 给定一个凸函数 $f: RR^n -> RR$, 在点 $x$ 处的次梯度集合 (i.e. 次微分) $partial f(x)$ 包含了所有满足
$
  f(y) >= f(x) + g^T (y - x), quad forall y in RR^n
$
的向量 $g$. 注意这里的 $g$ 不一定是唯一的, 因为在不可微点, 可能存在多个次梯度.

== 次梯度算法
为了最小化一个不可微的凸函数 $f$, 我们可以构造一个与梯度下降法形式上极其的似算法, 即选择初始点 $x_0$, 然后迭代
$
  x_(k+1) = x_k - alpha_k g_k
$
其中 $alpha_k > 0$ 是步长, $g_k$ 是从次微分 $partial f(x_k)$ 中 *任选* 的一个次梯度.

*步长选择* 是次梯度算法的关键. 常见的选择有:
1. 固定步长: $alpha_k = alpha > 0$;
2. 固定步距: $alpha_k dot.c norm(g_k) = beta$;
3. 消失步长: $alpha_k -> 0$ 且 $sum alpha_k = +oo$. 如 $alpha_k = 1 / k$.

== 次梯度算法的收敛性分析
=== 次梯度算法不是下降算法
与梯度下降法不同, 次梯度算法不一定是一个下降算法. 负梯度方向 $-nabla f(x)$ 保证了局部最速下降; 但是负次梯度方向 $-g_k$ 并不保证函数值下降, 它只保证 "指向" 包含最优解的半空间.

=== 衡量和最优解之间距离
我们先假设
1. $f$ 是凸函数且存在有限的最小值点 $x^*$;
2. $f$ 是 G-Lipschitz 连续的, 这意味着次梯度有界, 即存在常数 $G > 0$, 使得对于任意 $x in RR^n$ 和任意 $g in partial f(x)$, 有 $norm(g) <= G$.

设 $x^*$ 是 $f$ 的一个全局最小点, 我们有 $f^* = f(x^*)$. 则从迭代公式出发:
$
  || x_(k+1) - x^* ||^2 & = || x_k - alpha_k g_k - x^* ||^2 \
                        & = || x_k - x^* ||^2 - 2 alpha_k g_k^T (x_k - x^*) + alpha_k^2 || g_k ||^2
$
再利用次梯度的定义, 有
$
  f(x^*) >= f(x_k) + g_k^T (x^* - x_k) ==> g_k^T (x_k - x^*) >= f(x_k) - f(x^*)
$
考虑到 $norm(g_k) <= G$, 则有
$
  || x_(k+1) - x^* ||^2 <= || x_k - x^* ||^2 - 2 alpha_k (f(x_k) - f(x^*)) + alpha_k^2 G^2
$ <sub-gard-key-inequality>
这个不等式表明, 虽然 $f(x_k)$ 可能不单调下降, 但只要步长 $alpha_k$ 足够小, 使得
$
  2 alpha_k (f(x_k) - f(x^*)) >= alpha_k^2 G^2
  ==> alpha_k <= (2 (f(x_k) - f(x^*))) / G^2
$
那么, 我们就能保证点 $x_k$ 离最优解 $x^*$ 的距离在减少.

=== 步长策略的选择
我们把 @sub-gard-key-inequality 累加从 $i = 0$ 到 $k$, 得到核心的收敛关系:
$
  2 sum_(i = 0)^k alpha_i (f(x_i) - f(x^*)) <= || x_0 - x^* ||^2 + G^2 sum_(i = 0)^k alpha_i^2
$
如果我们采用固定步长 $alpha_k = alpha$, 则有
$
  min_(i = 0,...,k) f(x_i) - f(x^*) <= (|| x_0 - x^* ||^2 + (k + 1) alpha^2 G^2) / (2 (k + 1) alpha)
$
当 $k -> +oo$ 时, 右端项趋近于 $alpha G^2 / 2$, 这表明固定步长策略只能保证解的误差在一个常数范围内. 不保证收敛到最优解.

但是如果我们采用消失步长策略, 即 $alpha_k -> 0$ 且 $sum alpha_k = +oo$, 则 $alpha_i^2$ 会比 $alpha_i$ 衰减得更快, 因此右端项会趋近于零, 从而保证
$
  lim_(k -> +oo) min_(i = 0,...,k) f(x_i) = f(x^*)
$
所以为了保证次梯度算法收敛到最优解, 我们必须采用消失步长策略. 这是它和梯度下降法的一个重要区别.

= 牛顿法和拟牛顿法
== 牛顿法
=== 动机: 引入二阶信息
在梯度下降法中, 我们只利用了目标函数的一阶导数信息 (即梯度). 牛顿法通过引入二阶导数信息 (即 Hessian 矩阵) 来加速收敛.

=== 经典牛顿法
经典牛顿法将目标函数 $f(x)$ 在当前点 $x_k$ 处进行二阶泰勒展开:
$
  f(x_k + d) approx f(x_k) + nabla f(x_k)^T d + (1/2) d^T nabla^2 f(x_k) d
$
然后直接移动到这个二次近似的最小点, 即解方程
$
  nabla f(x_k) + nabla^2 f(x_k) d = 0
$
这就导出了牛顿方程
$
  nabla^2 f(x_k) d_k = -nabla f(x_k) ==> d_k = - (nabla^2 f(x_k))^(-1) nabla f(x_k)
$
这里解出来的 $d_k$ 就是 *牛顿方向*. 然后我们更新迭代:
$
  x_(k+1) = x_k + d_k = x_k - (nabla^2 f(x_k))^(-1) nabla f(x_k)
$
在经典牛顿法中, 我们通常选择步长 $alpha_k = 1$.

=== 经典牛顿法的收敛速度
经典牛顿法在目标函数 $f(x)$ 在最优点 $x^*$ 处满足以下条件时, 具有 $Q$-二次收敛速度:
1. $f(x)$ 二阶可微;
2. 在最优点处, $nabla f(x^*) = 0$ 且 $nabla^2 f(x^*)$ 是正定的;
3. Hessian 矩阵 $nabla^2 f(x)$ 在点 $x^*$ 处是 Lipschitz 连续的.

具体来说, 当迭代点 $x_k$ 足够接近最优点 $x^*$ 时, 有
$
  norm(x_(k+1) - x^*) <= C norm(x_k - x^*)^2
$
这里的常数 $C > 0$ 与 Hessian 的 Lipschitz 常数有关. 这意味着每次迭代后, 误差的数量级大约会平方, 导致非常快速的收敛. 这比梯度下降法的 $Q$-线性收敛要快得多.

=== 经典牛顿法的局限性
- *Hessian 矩阵非正定*: 如果 Hessian 矩阵 $nabla^2 f(x_k)$ 不是正定的, 则牛顿方向 $d_k$ 可能不是一个下降方向, 这会导致算法发散. (如 $f(x, y) = x^2 - y^2$ 在原点处的 Hessian 矩阵是非正定的.)

- *缺乏全局收敛性*: 经典牛顿法在初始点远离最优点时, 可能无法收敛到最优解. 通常需要结合线搜索或信赖域方法来增强全局收敛性.

- *巨大的计算开销*: 计算和存储 Hessian 矩阵的成本较高, 特别是在高维问题中. 这限制了牛顿法在大规模优化问题中的应用.

=== 修正牛顿法
修正牛顿法通过系统性地修改 Hessian 矩阵以保持其正定, 并且结合线搜索来确保全局收敛性.

具体来说, 在每次迭代中, 我们检查 Hessian 矩阵 $nabla^2 f(x_k)$ 是否正定. 如果不是, 则通过添加一个适当的正数倍的单位矩阵来修正它:
$
  H_k = nabla^2 f(x_k) + mu_k I
$
这里 $mu_k > 0$ 是一个调整参数, 使得 $H_k$ 成为正定矩阵. 然后我们解修正后的牛顿方程
$
  H_k d_k = -nabla f(x_k)
$
之后, 我们通过 Armijo 和 Wolfe 条件来选择步长 $alpha_k$, 并更新迭代:
$
  x_(k+1) = x_k + alpha_k d_k
$

上面 $mu_k$ 的计算有两种方式
1. *显式修正*: 直接计算 Hessian 的特征值, 并根据最小特征值调整 $mu_k$ 使得 $H_k$ 正定. 这种方法计算开销较大, 但能精确控制 Hessian 的正定性.
2. *隐式修正*: 使用 Cholesky 分解等数值方法, 在分解失败时动态调整 $mu_k$. 这种方法更高效, 在实际应用中更常用.

=== 非精确牛顿法
非精确牛顿法允许在求解牛顿方程时使用近似解, 以减少计算开销. 具体来说, 在每次迭代中, 我们不要求精确地解出
$
  nabla^2 f(x_k) d_k = -nabla f(x_k)
$
而是允许一个近似解 $d_k$ 满足
$
  || nabla^2 f(x_k) d_k + nabla f(x_k) || <= eta_k || nabla f(x_k) ||
$
这里 $eta_k in [0, 1)$ 是一个控制近似精度的参数. $eta_k$ 的选择会影响算法的收敛性:
- *$Q$-线性收敛*: 如果 $eta_k$ 有一个上界 $eta < 1$
- *$Q$-超线性收敛*: 如果 $eta_k -> 0$ 当 $k -> +oo$
- *$Q$-二次收敛*: 如果 $eta_k = O(norm(nabla f(x_k)))$

== 拟牛顿法
=== 动机: 避免计算 Hessian 矩阵
无论是修正牛顿法还是非精确牛顿法, 都需要显示或者隐式地计算 Hessian 矩阵 $nabla^2 f(x_k)$, 这在高维问题中计算开销较大 (尤其是参数量巨大的模型). 我们能否避免计算二阶导数, 同时又能利用二阶信息来加速收敛呢? 这就是拟牛顿法的出发点.

=== 用梯度历史去近似 Hessian 矩阵
拟牛顿法通过利用梯度的变化来构建 Hessian 矩阵的近似. 考虑 $f(x_k)$ 的一阶泰勒展开:
$
  nabla f(x_(k+1)) approx nabla f(x_k) + nabla^2 f(x_k) (x_(k+1) - x_k)
$
定义步长向量和梯度变化向量为
$
  s_k = x_(k+1) - x_k, quad y_k = nabla f(x_(k+1)) - nabla f(x_k)
$
则我们要求拟牛顿矩阵 $B_k$ 满足 *拟牛顿条件*:
$
  B_k s_k = y_k
$
这个条件要求拟牛顿矩阵 $B_k$ 能够正确地映射步长向量 $s_k$ 到梯度变化向量 $y_k$, 从而捕捉目标函数在该方向上的二阶信息.

=== 更新 $B_k$: BFGS 方法
BFGS (Broyden-Fletcher-Goldfarb-Shanno) 方法是一种常用的拟牛顿方法. 它通过以下公式更新拟牛顿矩阵 $ B_(k+1) = B_k + (y_k y_k^T) / (y_k^T s_k) - (B_k s_k s_k^T B_k) / (s_k^T B_k s_k) $
但是, 由于牛顿法需要计算 $B_k^(-1)$ 来获得搜索方向, 这里的求逆计算开销较大. 因此, 我们通常直接更新 $H_k = B_k^(-1)$, 其更新公式为
$
  H_(k+1) = (I - (s_k y_k^T) / (y_k^T s_k)) H_k (I - (y_k s_k^T) / (y_k^T s_k)) + (s_k s_k^T) / (y_k^T s_k)
$
这个迭代公式的核心性质是 *正定保持性*, 即如果初始矩阵 $H_0$ 是正定的, 则所有后续的 $H_k$ 也都是正定的, 前提是 $y_k^T s_k > 0$. 这个条件通常通过线搜索来保证.

=== BFGS 方法的收敛性
在适当的条件下, BFGS 方法具有良好的收敛性:
- *全局收敛性*: 如果目标函数 $f(x)$ 是凸的且其梯度是 Lipschitz 连续的, 则 BFGS 方法在结合线搜索时全局收敛到一个最优点.
- *超线性收敛*: 如果目标函数 $f(x)$ 在最优点处是二阶可微的, 且 Hessian 矩阵是正定的, 则 BFGS 方法在最优点附近具有超线性收敛速度.

=== L-BFGS 方法
L-BFGS (Limited-memory BFGS) 方法是 BFGS 方法的一个变种, 旨在减少存储和计算开销. 它通过只保留最近的 $m$ 次迭代的信息来近似 Hessian 矩阵, 而不是存储完整的 $n times n$ 矩阵, 其中 $n$ 是变量的维度.

具体来说, L-BFGS 方法只存储最近的 $m$ 对 $(s_k, y_k)$ 向量, 并使用这些信息来构建一个近似的逆 Hessian 矩阵 $H_k$. 这种方法大大减少了内存需求, 使得 L-BFGS 特别适合于大规模优化问题.
...
#align(center)[
  ...
  #table(
    ...
    fill: (x, y) => if y == 0 { rgb("#EFEFEF") },

    table.header([*特性*], [*梯度下降法*], [*经典牛顿法*], [*修正/非精确牛顿法\ (CG)*], [*BFGS*], [*L-BFGS*]),
    ...
    [*每步时间复杂度*], [$O(n)$], [$O(n^3)$], [$O(n^2)$ (Hessian-向量积)], [$O(n^2)$], [$O(m n)$],

    [*空间复杂度*], [$O(n)$], [$O(n^2)$], [$O(n^2)$ 或 $O(n)$], [$O(n^2)$], [$O(m n)$],

    [*收敛速度*], [Q-线性], [Q-二次], [Q-超线性/二次], [Q-超线性], [Q-超线性],

    [*全局收敛性*], [是 (有线搜索)], [否 (局部)], [是 (有线搜索)], [是 (有线搜索)], [是 (有线搜索)],
...
$
  x_(k + 1) = x_k + alpha_k d_k
$
移动时, 很容易跃出可行域. 为了处理这个问题, 我们可以将约束条件 "软化" 成目标函数的一部分, 通过引入一个罚函数来惩罚违反约束的解. 这样, 我们就可以将约束优化问题转化为一系列无约束优化问题.

== 等式约束和二次罚函数
考虑一个带等式约束的优化问题:
$
  min_(x) f_0 (x), quad "s.t." quad c_i (x) = 0, quad forall i in cal(E)
$
我们可以给原函数添加一个二次罚函数:
$
  P_E (x, sigma) = f_0(x) + sigma/2 sum_(i in cal(E)) c_i (x)^2
$
和 Lagrangian类似, 这里我们用平方项强制惩罚项为正.

由此, 我们给出二次罚函数法的算法流程:
1. 初始化: 选择初始点 $x_1$ 和罚参数 $sigma_1 > 0$, 设置罚因子增长率 $rho > 1$ 和迭代计数 $k = 1$.
2. 内部优化: 对于当前罚参数 $sigma_k$, 使用无约束优化方法 (如梯度下降法或牛顿法) 求解无约束优化问题 (以 $x_k$ 为起始点)
$
  min_(x) P_E (x, sigma_k)
$
3. 更新罚参数: 设内部优化的解为 $x_(k + 1)$, 更新罚参数
$
  sigma_(k + 1) = rho sigma_k
$
4. 检查收敛: 如果 $x_(k + 1)$ 满足收敛条件 (如约束违反度和目标函数变化足够小), 则停止; 否则, 设置 $k = k + 1$, 返回步骤 2.

这里 $sigma_k$ 序列必须单调增长并且趋向于无穷大, 以确保最终解满足约束条件.

== 处理不等式约束
对于带不等式约束的优化问题:
$
  min_(x) f_0 (x), quad "s.t." quad c_i (x) <= 0, quad forall i in cal(I)
$
我们可以定义一个不等式罚函数:
$
  P_I (x, sigma) = f_0 (x) + sigma/2 sum_(i in cal(I)) max(0, c_i (x))^2
$
这里, 只有当约束被违反时 (即 $c_i (x) > 0$), 罚函数才会对目标函数进行惩罚.

不过这里引入的 $max(0, c_i (x))$ 虽然是一阶可微的, 但是其二次导数在 $c_i (x) = 0$ 处不可微. 因此, 在实际应用中, 我们通常使用次梯度方法或光滑近似来处理这个问题.

== 对数障碍函数
另一种处理不等式约束的方法是使用对数障碍函数. 对于不等式约束 $c_i (x) <= 0$, 我们定义对数障碍函数为
$
  P_I (x, sigma) = f_0 (x) - sigma sum_(i in cal(I)) log(-c_i (x))
$
这个罚函数在约束边界 $c_i (x) = 0$ 处趋向于无穷大, 因此有效地阻止了解违反约束.

== $ell_1$ 精确罚函数
在二次罚函数中, 当 $sigma -> +oo$ 时, 罚函数法的解才会趋近于原始约束优化问题的解. 这可能导致数值不稳定和计算困难. 为了克服这个问题, 我们可以使用 $ell_1$ 精确罚函数:
$
  P (x, sigma) = f_0 (x) + sigma^* ( sum_(i in cal(E)) |c_i (x)| + sum_(i in cal(I)) max(0, c_i (x)) )
$
这里 $sigma^*$ 是一个足够大的常数. 这种罚函数具有 "精确性", 即当 $sigma >= sigma^*$ 时, 罚函数的最小点就是原始约束优化问题的最小点. 根据理论 $sigma^*$ 的上界可以通过 KKT 条件中的拉格朗日乘子来估计, 即
$
  sigma^* <= max ( max_(i in cal(E)) |lambda_i^*|, max_(i in cal(I)) mu_i^* )
$
...
我们回顾等式约束优化问题
$
  min_x quad f(x) quad "s.t." c_i (x) = 0, quad i in cal(E)
$
考虑二次罚函数法, 我们构造
$
  min_x P(x, sigma) = f(x) + sigma / 2 sum_(i in cal(E)) c_i^2 (x)
$
根据一阶最优性条件, 我们有
$
  c_i (x^(k + 1)) approx - lambda_i^* / sigma_k
$
为了确保可行性 $c_i(x) -> 0$, 我们必须让罚参数 $sigma_k -> oo$, 但是在这种情况下, 子问题的 Hessian 矩阵 $nabla^2 P(x, sigma)$ 会变得越来越病态, 导致数值算法极不稳定. 所以我们需要一种不必让罚参数趋于无穷的方法.

== 增广拉格朗日函数
我们可以结合标准 Lagrangian和罚函数项
$
  L_sigma (x, lambda) = f(x) + sum_(i in cal(E)) lambda_i c_i (x) + sigma / 2 sum_(i in cal(E)) c_i^2 (x)
$
现在在第 $k$ 步最小化 $L_(sigma k) (x, lambda^k)$, 最优解 $x^k$ 满足
$
  nabla f(x^(k + 1)) + sum_(i in cal(E)) [lambda_i^k + sigma_k c_i (x^(k + 1)) ] nabla c_i (x^(k + 1)) = 0
$
对比原问题的 KKT 条件
$
  nabla f(x^*) + sum_(i in cal(E)) lambda_i^* nabla c_i (x^*) = 0
$
我们期望
$
  lambda_i^* approx lambda_i^k + sigma_k c_i (x^(k + 1))
$
由此, 我们给出乘子的更新法则
$
  markrect(lambda^(k + 1) = lambda^k + sigma_k c(x^(k + 1)))
$
...
#algorithm-figure(
  ...
  vstroke: .5pt + luma(200),
  {
    ...
    Procedure(
      "AugmentedLagrange",
      ($x^0$, $lambda^0$, $sigma^0 > 0$, $rho > 1$, $eta_k > 0$),
      {
        For(
          $k = 0, 1, 2, ...$,
          {
            Comment[求解子问题: 以 $x^k$ 为初始点, 近似求解无约束优化问题]
            Assign[$x^(k + 1)$][$arg min_x L_(sigma_k) (x, lambda^k)$]
            ...
            LineBreak
            Comment[更新乘子]
            Assign[$lambda^(k + 1)$][$lambda^k + sigma_k c(x^(k + 1))$]

            LineBreak
            Comment[更新罚参数]
            Assign[$sigma^(k + 1)$][$rho sigma^k$]
          },
        )
      },
    )
...
*等式约束下的收敛性* \
设 $x^*$ 为 *原问题* 的局部最优解, 且在该点处满足二阶充分条件和LICQ, 则存在一个罚参数阈值 $bar(sigma)$, 使得对于任意的 $sigma > bar(sigma)$, $x^*$ 都是 *增广拉格朗日问题* 的一个严格局部最优解.

== 处理不等式约束
考虑更一般的不等式约束问题
$
     min quad & f(x) \
  "s.t." quad & c_i (x) = 0, quad i in cal(E) \
              & c_i (x) <= 0, quad i in cal(I)
$
这里我们可以引入松弛变量, 令 $c_i (x) <= 0 --> c_i (x) + s_i = 0$, 其中 $s_i >= 0$. 由此, 构造增广 Lagrangian
$
  L_sigma (x, s, lambda, mu) = f(x) + sum_(i in cal(E)) lambda_i c_i (x) + sum_(i in cal(I)) mu_i (c_i (x) + s_i) \
  + sigma / 2 [ sum_(i in cal(E)) c_i^2 (x) + sum_(i in cal(I)) (c_i (x) + s_i)^2 ], quad s_i >= 0
$
我们先固定 $x$, 解析地求出最优的 $s$:
$
  s_i^* = cases(
    0 & "if" mu_i + sigma c_i (x) >= 0,
    - (mu_i + sigma c_i (x)) / sigma & "if" mu_i + sigma c_i (x) < 0
  )
$
基于 $lambda$ 的更新法则 $lambda arrow lambda + sigma c(x)$, 对于不等式约束, 我们有类似的逻辑 $mu arrow mu + sigma (c(x) + s^*)$. 将 $s_i^*$ 代入, 我们惊奇地发现两种情况可以统一表达为:
$
  markrect(mu_i^(k + 1) = max(0, mu_i^k + sigma_k c_i (x^(k + 1))))
$
实际上, 通过代入 $s_i^*$, 我们可以消去松弛变量, 得到仅依赖于 $x$ 的 *P-H-R 形式* 增广 Lagrangian:
$
