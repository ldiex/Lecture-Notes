...
$
  ...
  P & = { S -> R | "-" R | "+" R, \
    ...
    & quad N -> 0 | A M, \
    & quad D -> 0 | M A, \
    & quad A -> 1 | 2 | 3 | 4 | 5 | 6 | 7 | 8 | 9, \
    & quad M -> A | A M | epsilon \
  }
$
...
其中
- $Q$ 是有限状态集合.
- $Sigma$ 是输入符号的有限集合 (输入字母表), 不包含空符号 $B$.
- $Gamma$ 是带符号的有限集合 (带字母表), 包含 $Sigma union {B} subset.eq Gamma$.
- $delta: Q times Gamma -> Q times Gamma times { L, R }$ 是状态转移函数.
- $q_0 in Q$ 是初始状态.
- $q_"acc" in Q$ 是接受状态.
- $q_"rej" in Q$ 是拒绝状态, 且 $q_"acc" != q_"rej"$.

*图灵机的计算过程* \
一个 TM $M$ 接受输入串 $w in Sigma^(*)$, 如果存在一个状态序列 $r_0, r_1, r_2, ..., r_n in Q$ 和带内容序列 $s_0, s_1, s_2, ..., s_n in Gamma^(*)$ 满足:
1. 初始条件: $r_0 = q_0$ 且 $s_0 = B w B B B ...$.
2. 转移条件: 对于每个 $i$ (其中 $0 <= i < n$), 存在 $( r_(i+1), X, D ) = delta(r_i, Y)$, 其中 $Y$ 是带头当前所在位置的符号, 并且 $s_(i+1)$ 是通过将带头位置的符号 $Y$ 替换为 $X$, 然后将带头向左或右移动一格 (由 $D$ 指定) 得到的带内容.
3. 接受条件: $r_n = q_"acc"$.

我们用 $L(M)$ 表示图灵机 $M$ 接受的语言.

== 图灵机的格局
类似于 PDA 的即时描述, TM 的格局 (Configuration) 表示 TM 在某一时刻的 *状态*. 格局可以被表示为
$
  u q a v
$
其中
- $q$ 是当前状态.
- 当前的带内容为 $u a v$, 其中 $u, v in Gamma^(*)$, $a in Gamma$.
- 带头当前位于符号 $a$ 上.

由此我们可以给出 TM 的几个特殊格局:
- *初始格局*: $q_0 w$ 这里 $w in Sigma^(*)$ 是输入串.
- *接受格局*: $u q_"acc" a v$.
- *拒绝格局*: $u q_"rej" a v$.
- *停机格局*: 接受和拒绝格局都是停机格局, 因为在这些格局下 TM 不会继续进行状态转移.


*格局的产生* \
如果图灵机 $M$ 能够合法地从格局 $C_1$ 转移到格局 $C_2$, 则称 $C_2$ 是由 $C_1$ *产生* 的, 记作 $C_1 tack.r_M C_2$.

形式化地说:
- 如果 $delta(q_i, b) = (q_j, c, L)$, 则:
$
  u a q_i b v & tack.r_M u q_j a c v \
      q_i b v & tack.r_M q_j c v quad ("If already at the left end of the tape")
$
- 如果 $delta(q_i, b) = (q_j, c, R)$, 则:
$
  u a q_i b v & tack.r_M u a c q_j v
$

== 图灵机的计算结果和可判定性
在输入上运行一个图灵机时, 可能出现三个结果:
1. *接受*: 如果存在一个接受格局 $C_f$ 使得 $( q_0 w ) tack.r_(M^*) C_f$, 则称 $M$ 接受输入串 $w$.
2. *拒绝*: 如果存在一个拒绝格局 $C_f$ 使得 $( q_0 w ) tack.r_(M^*) C_f$, 则称 $M$ 拒绝输入串 $w$.
3. *不停机 (循环)*: 如果 $M$ 既不接受也不拒绝输入串 $w$, 则称 $M$ 在输入 $w$ 上不停机 (循环).

如果一个图灵机对所有输入都能停机 (即要么接受, 要么拒绝), 则称其为 *判定器* (Decider). 反之, 如果一个图灵机在某些输入上可能不停机, 则称其为 *识别器* (Recognizer).

如果一个语言 $A$ 是 *图灵可判定的* (Turing Decidable), 则存在一个判定器 $M$ 使得 $L(M) = A$. 对于 $forall x in.not A$, $M$ 拒绝 $x$; 对于 $forall x in A$, $M$ 接受 $x$.

如果一个语言 $A$ 是 *图灵可识别的* (Turing Recognizable), 则存在一个识别器 $M$ 使得 $L(M) = A$. 对于 $forall x in A$, $M$ 接受 $x$; 对于 $forall x in.not A$, $M$ 要么拒绝 $x$, 要么不停机.

== 图灵机的等价变形
*自动机类的等价性* \
两个自动机是等价的, 如果它们接受的语言相同.

考虑两个自动机类 $C_1$ 和 $C_2$. 如果对于任意 $M_1 in C_1$, 存在一个 $M_2 in C_2$ 使得 $L(M_1) = L(M_2)$, 则称 $C_2$ *不弱于* $C_1$. 如果 $C_1$ 和 $C_2$ 互相不弱于对方, 则称它们是 *等价的*.

*允许读写头保持不动的图灵机* \
标准图灵机的读写头每次只能向左或向右移动一格, 那如果我们允许读写头保持不动 (Stay) 呢? 这种变形的图灵机和标准图灵机是等价的. 因为我们可以通过先后向右和向左移动来模拟保持不动的操作, 反之亦然.

*模拟* \
给定自动机 $M$, 令 $d_0, d_1, ..., d_n$ 为 $M$ 的格局. 且有
$
  d_0 tack.r_M d_1 tack.r_M d_2 tack.r_M dots.c tack.r_M d_n
$
那我们称 $hat(M)$ 能 *模拟* $M$, 如果 $hat(M)$ 能执行 $M$ 的类似计算
$
  hat(d_0) tack.r_(hat(M)^*) hat(d_1) tack.r_(hat(M)^*) hat(d_2) tack.r_(hat(M)^*) dots.c tack.r_(hat(M)^*) hat(d_n)
$
其中 $hat(d_i)$ 为 $hat(M)$ 的格局, 且每个 $hat(d_i)$ 唯一对应 $M$ 的一个格局.

换言之, 如果 $hat(M)$ 能够模拟 $M$ 的计算, 那么给定相应的起始格局, 就能从 $hat(M)$ 的计算序列中精确恢复出 $M$ 的计算序列.

注意 $hat(M)$ 可能需要多步来模拟 $M$ 的一步计算, 但最终它们的计算结果是相同的. 如果 $hat(M)$ 能够模拟 $M$, 则它们接受相同的语言, 即 $L(hat(M)) = L(M)$. 要证明两个自动机等价, 只需要证明两类自动机能互相模拟即可.

== 多带图灵机
*定义* \
多带图灵机 (Multi-tape Turing Machine) 具有多个读写带和对应的读写头. 一个 $k$ 带图灵机定义为一个七元组
$
  M = ( Q, Sigma, Gamma, delta, q_0, q_"acc", q_"rej" )
$
其转移函数为
$
  delta: Q times Gamma^k -> Q times ( Gamma times { L, R } )^k
$
也就是说, 在每一步转移中, 多带图灵机可以同时读取 $k$ 个带上的符号, 并且可以同时在 $k$ 个带上进行写入和移动操作.

在初始情况下, 第一带包含输入串, 其余 $k-1$ 带均为空 (只包含空符号 $B$). 多带图灵机的接受和拒绝条件与单带图灵机相同.

*和单带图灵机的等价性* \
多带图灵机和单带图灵机是等价的. 也就是说, 对于任意一个多带图灵机 $M$, 存在一个等价的单带图灵机 $M'$, 使得 $L(M) = L(M')$. 反之亦然.

为了将一个多带图灵机 $M$ 转换为等价的单带图灵机 $M'$, 我们可以采用以下方法:
1. *带内容的编码*: 在单带图灵机 $M'$ 上, 我们可以使用特殊符号 (例如 `#`) 来分隔不同带的内容. 例如, 如果多带图灵机 $M$ 有两带, 且当前状态下第一带的内容为 $u_1$ 且读写头位于符号 $a_1$, 第二带的内容为 $u_2$ 且读写头位于符号 $a_2$, 则在单带图灵机 $M'$ 上, 我们可以将带内容编码为
$
  B u_1 a_1 u_1^' \# u_2 a_2 u_2^' B
$
其中 $u_1^'$ 和 $u_2^'$ 分别是第一带和第二带读写头右侧的内容.

2. *模拟转移*: 对于多带图灵机 $M$ 的每一步转移, 单带图灵机 $M'$ 需要执行多个步骤来模拟. 具体来说, $M'$ 需要:
  - 扫描整条带, 找到每个带的读写头位置, 并读取相应的符号.
  - 根据多带图灵机 $M$ 的转移函数, 确定新的状态和每个带上要写入的符号以及读写头的移动方向.
  - 再次扫描整条带, 更新每个带上的符号并移动读写头位置.

*离线图灵机* \
离线图灵机 (Off-line Turing Machine) 是一种特殊的多带图灵机, 它只有一条输入带和多条工作带. 输入带在计算过程中只能读取, 不能写入或修改; 而工作带则可以自由读写和移动读写头.

== 非确定性图灵机
*定义* \
非确定性图灵机 (Nondeterministic Turing Machine, *NTM*) 与确定性图灵机类似, 但其转移函数允许多个可能的转移. 一个 NTM 定义为一个七元组
$
  M = ( Q, Sigma, Gamma, delta, q_0, q_"acc", q_"rej" )
$
其中转移函数为
...
$
这里 $P$ 表示幂集, 即对于任意状态 $q in Q$ 和带符号 $X in Gamma$, $delta(q, X)$ 可以包含多个可能的三元组 $( p, Y, D )$, 表示 NTM 可以转移到状态 $p$, 在带上写入符号 $Y$, 并将读写头向左或向右移动 (由 $D$ 指定).

类似于 NFA, NTM 在每一步计算中可以 "分叉" 出多个计算路径. 如果存在至少一条计算路径能够接受输入串 $w$, 则称 NTM 接受 $w$.

*和确定性图灵机的等价性* \
非确定性图灵机和确定性图灵机是等价的. 也就是说, 对于任意一个 NTM $M$, 存在一个等价的确定性图灵机 $M'$, 使得 $L(M) = L(M')$. 反之亦然.

我们可以通过构造一个确定性图灵机 (DTM) 来模拟 NTM 的所有可能计算路径. 我们把 NTM 所有可能的配置组织成一棵计算树, DTM 通过广度优先搜索 (BFS) 的方式遍历这棵树, 检查每个配置是否为接受配置. 如果找到一个接受配置, 则 DTM 接受输入串; 如果遍历完整棵树都没有找到接受配置, 则 DTM 拒绝输入串.

具体来说, 我们构造的 DTM 需要:
1. 在纸带上维护一个 "配置队列", 用于存储 NTM 的所有可能配置. (比如说用 `#` 分隔不同配置)
2. 初始时, 队列中只包含 NTM 的初始配置.
3. 在每一步中
  - 从队列头部取出一个配置 $C$
  - 检查 $C$ 是否为接受配置, 如果是则接受输入串.
  - 否则, 计算 NTM 在配置 $C$ 下的所有可能转移, 并将得到的新配置添加到队列尾部.
4. 如果队列为空且没有找到接受配置, 则拒绝输入串.

*枚举器* \
一个 *枚举器* (Enumerator) 是一种特殊的图灵机, 它没有输入带, 只有一个输出带. 枚举器通过不断地向输出带写入字符串来 "枚举" 一个语言中的所有字符串.

枚举器以空白输入的工作带开始运行. 如果不停机 (即没有进入接受或拒绝状态), 可能会打印出一个无限序列的串.


#figure(
  align(center)[#table(
    columns: (auto, auto, auto, auto),
    table.header([*计算装置*], [*输入*], [*输出*], [*描述*]),
    table.hline(),
    [识别器], [$x$], [$0 \/ 1 \/ ?$], [确定 $x$  是否属于语言 $L$],
    [判定器], [$x$], [$0 \/ 1$], [确定 $x$  是否属于语言 $L$],
    [枚举器], [$epsilon$], [$w_1, w_2, w_3, ...$], [枚举语言 $L$ 中的所有字符串],
    [转换器], [$x$], [$y$], [将 $x$ 转换为 $y$, 其中 $x in L_1$, $y in L_2$],
  )],
)

*枚举器与图灵可识别语言的等价性* \
一个语言 $L$ 是图灵可识别的, 当且仅当存在一个枚举器 $E$ 能够枚举 $L$ 中的所有字符串.

可识别 $==>$ 可枚举: \
假设语言 $L$ 是图灵可识别的, 则存在一个识别器 $M$ 使得 $L(M) = L$. 我们可以构造一个枚举器 $E$ 来枚举 $L$ 中的所有字符串:
1. 枚举所有可能的字符串 $w_1, w_2, w_3, ...$ (例如按长度递增的字典序).
2. 对于每个字符串 $w_i$, 运行识别器 $M$ 来检查 $w_i$ 是否属于 $L$.
3. 如果 $M$ 接受 $w_i$, 则将 $w_i$ 输出到输出带上.
*注*: 为了避免 $M$ 在某些字符串上不停机, 我们可以采用 "并行模拟" 的方式, 即同时运行多个 $M$ 的实例, 每个实例处理一个字符串, 并交替执行每个实例的一步计算.

可枚举 $==>$ 可识别: \
假设存在一个枚举器 $E$ 能够枚举语言 $L$ 中的所有字符串. 我们可以构造一个识别器 $M$ 来识别 $L$:
1. 给定输入字符串 $w$, 运行枚举器 $E$.
2. 每当 $E$ 输出一个字符串 $w_i$, 检查 $w_i$ 是否等于 $w$.
3. 如果找到了 $w_i = w$, 则 $M$ 接受 $w$.
4. 如果 $E$ 不断输出字符串而没有找到 $w$, 则 $M$ 不停机.

== 形式语言与自动机
图灵机的概念具有很强的稳健性，仅仅通过构造更复杂的带子结构 (例如多维带、多带, 或者引入不确定性) 并不能扩展图灵机的计算. 然而, 通过对带子的使用方式施加限制, 可以有效地制约图灵机的计算能力.

- 下推自动机 (PDA): PDA可以被视为一种受限的非确定型图灵机, 其带子的读写操作采用栈的方式 (压栈对应写, 弹栈对应读).

- 受限的离线图灵机: 如果一个离线图灵机在运行中只允许使用工作带上的一个有限区域, 此时它对应于一个有穷状态机.

*线性有界自动机 (LBA) 与上下文有关语言 (CSL)* \
LBA不允许其读写头离开包含输入字符串的带子区域. 如果机器试图将读写头移出输入的两个端点, 读写头将停在原地不动. 与受限的离线图灵机不同的是, LBA允许在输入区域内自由地读写和修改符号.

LBA的能力比 PDA 更为强大, 但仍然弱于通用图灵机. LBA能够识别的语言称为 *上下文有关语言* (Context-Sensitive Language, *CSL*). CSL包含了所有的CFL, 但并不包含所有的递归可枚举语言.

*图灵机 (TM) 与短语结构文法 (PSG)* \
通用图灵机能够识别所有的递归可枚举语言 (Recursively Enumerable Language, *REL*). 这些语言可以通过 *短语结构文法* (Phrase Structure Grammar, *PSG*) 来描述. PSG允许产生式的左侧和右侧都包含任意的符号串, 包括终结符和非终结符. 它就是我们一开始提到的 "一般文法" (Unrestricted Grammar).

#figure(
  align(center)[#table(
    columns: (auto, auto, auto, auto),
    table.header([*语言类别*], [*自动机模型*], [*能力对比*], [*限制/特点*]),
    table.hline(),
    [正则语言 (RL) (Type 3)], [有限状态机 (DFA/NFA)], [最弱], [无存储空间],
    [上下文无关语言 (CFL) (Type 2)], [下推自动机 (PDA)], [中等], [使用栈来记忆信息 (LIFO)],
    [上下文有关语言 (CSL) (Type 1)], [线性有界自动机 (LBA)], [较强], [带子长度受限于输入长度],
    [短语结构语言 (PSL) (Type 0)], [图灵机 (TM)], [最强], [无限带子, 可读写任意位置],
  )],
)

= 语言的判定
== 算法的定义
非形式化地说, 算法是为实现某个任务而构造的简单指令集. 它描述了一个计算过程, 该过程具备以下特征:
1. 从一个初始状态和初始输入 (可能为空) 开始执行
2. 经过一系列有限而清晰定义的状态
3. 最终产生输出并停止于一个终态

在计算理论中, 算法被认为是与 *处处停机的图灵机* 等价的. 这里的 "处处停机的图灵机" 特指对所有输入都停机的图灵机, 也被称为判定器 (decider)

== 可判定性问题
*与正则语言相关的可判定性问题* \
正则语言 (由 DFA, NFA 或正则表达式描述) 相关的几个核心问题都是可判定的:

#figure(
  align(center)[#table(
    columns: (auto, auto, auto, auto),
    table.header([*问题类型*], [*语言形式*], [*判定结论*], [*可判定性*]),
    table.hline(),
    [DFA 接受性问题], [$A_"DFA" = angle.l B, w angle.r$], [$B$ 是 DFA 且接受 $w$], [可判定],
    [NFA 接受性问题], [$A_"NFA" = angle.l B, w angle.r$], [$B$ 是 NFA 且接受 $w$], [可判定],
    [正则表达式接受性问题], [$A_"RE" = angle.l B, w angle.r$], [$B$ 是正则表达式且接受 $w$], [可判定],
    [DFA 空性问题], [$E_"DFA" = angle.l B angle.r$], [$B$ 是 DFA 且 $L(B) = emptyset$], [可判定],
    [DFA 等价性问题], [$E Q_"DFA" = angle.l A, B angle.r$], [$A, B$ 是 DFA 且 $L(A) = L(B)$], [可判定],
  )],
)

这里, NFA 和 REX 的接受性问题都可以归结为 DFA 的接受性问题. 那么为什么 DFA 接受性问题是可判定的呢? 这是因为 $w$ 是有限的且 DFA 是确定的, 所以 DFA 跑完 $w$ 之后停在的状态也是确定的.

对于 DFA 的空性问题, 我们可以把 DFA 看成一张有向图. 问 $L(A)$ 是否为空, 就是问图中是否存在从初始状态到接受状态的有向路径. 这可以通过一个 BFS 或者 DFS 来解决.

对于等价性问题. 我们可以利用封闭性构造对称差
$
  L(C) := [L(A) inter overline(L(B))] union [L(B) inter overline(L(A))]
$
那么我们就有 $L(C) = emptyset$ 当且仅当 $L(A) = L(B)$. $L(C)$ 的构造可以通过 DFA 的非交并运算获得, 所以等价性问题可以被转化为空性问题.

*与上下文无关语言相关的可判定性问题* \
上下文无关语言 (CFL) 相关的可判定性问题结论有所不同:
...
#figure(
  align(center)[#table(
    ...
    table.header([*问题类型*], [*语言形式*], [*判定结论*], [*可判定性*]),
    table.hline(),
    [CFG 派生性问题], [$A_"CFG" = angle.l G, w angle.r$], [CFG $G$ 派生 $w$], [可判定],
    [CFG 空性问题], [$E_"CFG" = angle.l G angle.r$], [CFG $G$ 派生空字符串], [可判定],
    [CFG 等价性问题], [$E Q_"CFG" = angle.l G, H angle.r$], [CFG $G$ 等价于 $H$], [*不*可判定],
  )],
)

CFG 的派生性问题先要把它转化为 CNF, 这样每一步推导都会增加字符串长度. 如果要生成长度为 $n$ 的串, 推导步数是固定的 $(2n - 1)$, 这样就可以在有限步骤下模拟 CFG 的推导过程.

对于 CFG 的空性问题, 我们可以通过 *标记法* 来解决. 首先标记所有的终结符. 然后重复扫描产生式: 如果产生式 $A -> U_1 ... U_k$ 右边的所有符号都已被标记, 则标记变量 $A$. 重复直到没有新变量被标记. 如果开始变量 $S$ 被标记, 则语言非空.

对于 CFG 的等价性问题, 它是 *不可判定* 的. 这里的关键区别在于 CFL 对 *交集* 和 *补集* 不封闭. 因此我们无法像 DFA 那样构造对称差语言 $L(C) = (L(G) inter overline(L(H))) union (L(H) inter overline(L(G)))$ 并检查其空性. 事实上, 没有任何算法能解决一般的 CFG 等价性问题.
...
== 利用计算历史的归约
=== 计算历史
设 $M$ 是一个图灵机, $w$ 是其输入串. $M$ 在输入 $w$ 上的计算历史 (Computation History) 是一系列格局的序列
$
  C_1, C_2, C_3, ..., C_k
$
其中 $C_1$ 必须是 $M$ 在 $w$ 上的初始格局, $C_k$ 必须是一个停机格局, 且对于每个 $1 <= i < k$, $C_i$ 产生 $C_(i+1)$ (即 $C_i tack.r_M C_(i+1)$).

如果 $C_k$ 是一个接受格局, 则称该计算历史为 *接受计算历史*; 如果 $C_k$ 是一个拒绝格局, 则称该计算历史为 *拒绝计算历史*.

=== 线性有界自动机 (LBA) 的相关判定性
LBA 是存储空间受输入长度 $n$ 线性限制的图灵机. 

*$A_"LBA"$ 是可判定的*: \
对于带长为 $n$, 状态数为 $q$, 符号表大小为 $s$ 的 LBA, 其可能的格局总数至多为 $q times s^n times n$. 因此, 如果 LBA 在 $n times q times s^n$ 步内没有停机, 则它必然进入了一个循环. 基于此, 我们可以构造一个判定器来解决 $A_"LBA"$:
1. 给定输入 $angle.l M, w angle.r$, 计算 $n = |w|$.
2. 构造一个计数器, 初始化为 $0$.
3. 模拟 LBA $M$ 在输入 $w$ 上的计算, 每执行一步, 计数器加 $1$.
4. 如果 $M$ 在 $n times q times s^n$ 步内进入接受状态, 则接受 $angle.l M, w angle.r$.
5. 如果 $M$ 在 $n times q times s^n$ 步内进入拒绝状态, 则拒绝 $angle.l M, w angle.r$.
6. 如果计数器达到 $n times q times s^n$ 而 $M$ 仍未停机, 则拒绝 $angle.l M, w angle.r$.

*$E_"LBA"$ 是不可判定的*: \
我们可以通过归约来证明 $E_"LBA"$ 是不可判定的. 假设存在一个判定器 $D$ 能够解决 $E_"LBA"$, 那么我们可以构造一个 $A_"TM"$ 的判定器 $B$. 这个 $B$ 仅识别 $M$ 在 $w$ 上的接受计算历史 $C_1, C_2, C_3, ..., C_k$.

当 $B$ 收到一个候选字符串 $x$ (形式为 $C_1 \# C_2 \# dots.c \# C_k$), 它会检查:
1. 初始格局是否正确 (即 $C_1$ 是否为 $M$ 在 $w$ 上的初始格局).
2. 每个格局 $C_i$ 是否合法地转移到 $C_(i+1)$ (即 $C_i tack.r_M C_(i+1)$).
3. 最终格局 $C_k$ 是否为接受格局.

虽然 LBA 的存储空间受限, 但它足以完成上述检查. 因为待检查的字符串 $x$ 已经包含了完整的格局序列. $B$ 只需要逐个验证每个格局的合法性, 并不需要额外的存储空间来模拟 $M$ 的计算.

通过这种构造, 我们建立了两者的等价关系:
- 如果 $M$ 接受 $w$, 则存在一个接受计算历史, 因此 $B$ 会接受某个字符串 $x$, 即 $L(B)$ 非空.
- 如果 $M$ 不接受 $w$, 则不存在任何接受计算历史, 因此 $B$ 不会接受任何字符串, 即 $L(B)$ 为空.

所以如果能判定 $B$ 的语言是否为空, 就能判定 $M$ 是否接受 $w$. 这与 $A_"TM"$ 的不可判定性矛盾. 因此, $E_"LBA"$ 也是不可判定的.
