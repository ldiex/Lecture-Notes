...
立体视觉的本质是三角测量 (Triangulation). 当你在一辆飞驰的火车上看窗外, 近处的栏杆飞速后退, 远处的山脉却纹丝不动. 这种 "移动量" 的差异就是 *视差*.

在最简单的 *平行双目系统* (左右相机光轴平行, 仅沿 X 轴平移 $T_x$) 中, 深度 $Z$ 和视差 $d$ 成反比:
$
  Z = (f T_x) / d
$
这意味着: 物体越近, 视差越大; 物体越远, 视差趋近于零. 通过测量图像中同一物体在左右图像上的位置差 (视差), 我们可以计算出物体的实际深度. 这就是立体视觉的核心原理, 它使得计算机能够从二维图像中恢复出三维场景的信息.

不过这里也有一个问题, 我们怎么知道左图中的这个像素, 对应右图中的哪一个像素? 这就是 *立体匹配* (Stereo Matching) 问题. 一个常用的办法是利用滑动窗口进行 *归一化互相关 (NCC)* 计算, 找到最佳匹配点. 具体来说, 对于左图中的一个像素点, 我们在右图的同一扫描线上滑动一个窗口, 计算窗口内像素的 NCC 分数, 选择分数最高的位置作为匹配点. 通过这种方式, 我们可以为每个像素点找到对应的视差值, 从而构建出完整的视差图 (Disparity Map).

== 极几何 (Epipolar Geometry)
平行双目系统中, 由于相机光轴平行, 视差仅在水平方向上变化. 但在更一般的双目系统中, 相机可能存在旋转和平移, 这时视差不再局限于水平线. 为了简化匹配过程, 我们引入了 *极几何* 的概念.

=== 极几何约束
在双目匹配中, 我们最头疼的是要在右图中茫茫多的像素里找左图的对应点. 但极几何告诉我们, 左相机的中心 $O$, 右相机的中新 $O'$ 和左图中的点 $P$ 共面. 这意味着, 右图中对应点 $P'$ 必须落在通过 $O'$ 和 $P$ 的平面上. 这个平面与右图像平面的交线就是 *极线 (Epipolar Line)*. 因此, 对于左图中的每个点, 我们只需要在右图的极线上搜索对应点, 将二维搜索问题简化为一维搜索问题.

=== 本质矩阵 (Essential Matrix)
本质矩阵 $E$ 是描述两个相机坐标系之间关系的一个 $3 times 3$ 矩阵. 它将左图像中的点 $x$ 映射到右图像中的极线 $l'$ 上:
$
  l' = E x
$
本质矩阵可以通过相机的旋转矩阵 $R$ 和平移向量 $t$ 计算得到:
$
  E = [t]_x R
$
其中 $[t]_x$ 是平移向量 $t$ 的反对称矩阵表示, 即
$
  [t]_x = mat(
    delim: "[",
    0, -t_z, t_y;
...
$
本质矩阵有 $5$ 个自由度 (3 个旋转 + 3 个平移 - 1 个尺度), 因此至少需要 $5$ 对匹配点来唯一确定它.

=== 基础矩阵 (Fundamental Matrix)
基础矩阵 $F$ 是本质矩阵在像素坐标系下的表示. 它同样是一个 $3 times 3$ 矩阵, 将左图像中的点 $x$ 映射到右图像中的极线 $l'$ 上:
$
  l' = F x
$
基础矩阵与本质矩阵的关系通过相机内参矩阵 $K$ 和 $K'$ 表示:
$
  F = K'^(-T) E K^(-1)
$
基础矩阵有 $7$ 个自由度, 其中 $5$ 个来自本质矩阵, 另外 $2$ 个来自内参矩阵的比例因子. 因此, 至少需要 $7$ 对匹配点来唯一确定基础矩阵.

== 基础矩阵的估计方法
=== 归一化八点法 (Normalized Eight-Point Algorithm)
归一化八点法是一种经典的估计基础矩阵 $F$ 的方法. 它的核心思想是通过线性方程组求解 $F$,然后进行奇异值分解 (SVD) 来强制 $F$ 的秩为 $2$. 具体步骤如下:
1. *数据归一化*: 对输入的匹配点 (通过特征匹配方法获得) 进行归一化处理, 使得点的均值为零, 均方根距离为 $sqrt(2)$. 这一步有助于提高数值稳定性.
2. *构建线性方程组*: 对于每一对匹配点 $(x_i, x_i')$, 构建一个线性方程 $x_i'^T F x_i = 0$. 将所有匹配点的方程堆叠起来, 得到一个矩阵方程 $A f = 0$, 其中 $f$ 是基础矩阵 $F$ 的向量化形式.
3. *求解线性方程组*: 通过奇异值分解 (SVD) 求解矩阵 $A$, 得到基础矩阵 $F$ 的初始估计.
4. *强制秩为 2*: 对初始估计的 $F$ 进行奇异值分解, 将最小的奇异值设为零, 然后重新构建 $F$.
5. *反归一化*: 将 $F$ 反归一化, 得到最终的基础矩阵.

=== RANSAC
初始的特征匹配 (如 SIFT/ORB) 会有很多误匹配 (Outliers), 直接用这些点估计基础矩阵会导致结果不准确. RANSAC (Random Sample Consensus) 提供了一种鲁棒的方法来估计基础矩阵, 它通过迭代随机采样和模型验证来剔除误匹配点. 具体步骤如下:
1. *随机采样*: 从匹配点集中随机选择 $8$ 对点
2. *模型估计*: 使用归一化八点法估计基础矩阵 $F$.
3. *内点验证*: 计算所有匹配点到估计的基础矩阵的距离, 判断哪些点是内点 (距离小于某个阈值).
4. *统计内点数量*: 记录内点数量, 如果当前内点数量超过之前的最大值, 则更新最佳模型.
5. *重复迭代*: 重复上述过程若干次, 最终选择内点数量最多的基础矩阵作为最终结果.

== 相机定位
相机定位的目标是通过相机在不同视角下拍摄的图像, 估计出相机在三维空间中的位置和朝向 (即外参 $R$ 和 $t$). 这对于增强现实 (AR), 机器人导航和三维重建等应用至关重要.

=== PnP 问题 (Perspective-n-Point)
PnP 问题是指已知 $n$ 个三维点的世界坐标及其对应的在二维图像平面上的投影坐标, 同时相机的内参 $K$ 已知, 求解相机的外参 $R$ 和 $t$.
...
*P3P 问题的多解性*:
P3P 问题通常有多解, 因为对于同一组三维点, 可以有多种相机姿态满足投影关系. 具体来说,
- 当 $n = 3$ 时, P3P 问题有 $4$ 个解, 对应于四种不同的相机姿态. 这四个解分别对应于四个不同的相机位置和方向, 但它们在三维空间中可能是等价的 (对于二维空间, 我们知道在平面上给定两点 $A$, $B$ 和一个视角角 $alpha$, 所有能看到 $A B$ 成角 $alpha$ 的点, 落在以 $A B$ 为弦的两个圆弧上. 现在如果再加第三个点 $C$ 和另一个视角角 $beta$, 这两个圆弧最多会相交于 $4$ 个点, 这就是 P3P 的多解性).

*DLT (Direct Linear Transform)*:
在拥有至少 $6$ 对匹配点的情况下, 可以使用 DLT 方法来求解 PnP 问题. DLT 通过构建一个线性方程组, 然后使用奇异值分解 (SVD) 来求解相机的外参.

== 目标定位
目标定位是一个 6D 的物体姿态估计问题, 目标是通过相机拍摄的图像, 估计出物体在三维空间中的位置和朝向.

=== 直接法 (End-to-End)
直接法是指通过深度学习模型, 直接从图像中预测物体的位姿. 这种方法通常需要大量的标注数据来训练模型, 以便模型能够学习到物体的几何特征和外观变化. 直接法的优点是可以处理复杂的场景和多样化的物体外观, 但缺点是需要大量的数据和计算资源.

=== 基于 PnP 的方法
基于 PnP 的方法是指通过已知的三维模型和图像中的特征点匹配, 使用 PnP 算法来估计物体的位姿. 这种方法通常需要预先知道物体的三维模型, 并通过特征匹配来获取图像中的关键点. 基于 PnP 的方法的优点是可以在已知模型的情况下快速估计物体的位姿, 但缺点是对物体的外观变化和遮挡较为敏感.

= 3D 重建
== 相机标定
=== 回顾相机模型
$
  s bold(m) = K [R | t] bold(M)_w
$
其中
- $bold(M) = (X, Y, Z, 1)^T$ 是三维世界坐标系中的点的齐次坐标.
- $[R | t]$ 代表相机的外参矩阵, 描述了相机在世界坐标系中的位置和朝向. 它的具体形式为
$
  [R | t] = mat(
    delim: "[",
    r_11, r_12, r_13, t_x;
...
$
其中 $R$ 是一个 $3 times 3$ 的旋转矩阵 (自由度为 $3$), $t$ 是一个 $3 times 1$ 的平移向量 (自由度为 $3$). 因此, 外参矩阵共有 $6$ 个自由度. 在齐次坐标下, 我们可以用一个矩阵完成一个仿射变换 (旋转 + 平移). 把世界坐标系的点 $bold(M)$ 变换到相机坐标系下的点 $bold(M)_c = (X_c, Y_c, Z_c)^T$.

- $K$ 是相机的内参矩阵, 描述了相机的内部属性, 包括焦距和主点位置. 它的具体形式为
$
  K = mat(
    delim: "[",
    f_x, 0, o_x;
...
$
其中, $f_x$ 和 $f_y$ 分别是以像素为单位的焦距, $o_x$ 和 $o_y$ 是主点的像素坐标. 内参矩阵共有 $4$ 个自由度. 它把相机坐标系下的点 $bold(M)_c$ 投影到图像平面上的点
$
  s bold(m) = mat(
    delim: "[",
    f_x X_c + o_x Z_c;
...
- 最后, 令 $s = Z_c$, 我们可以得到图像平面上的像素坐标
$
  u = (f_x X_c) / Z_c + o_x, quad v = (f_y Y_c) / Z_c + o_y
$


=== 投影矩阵
有了 $K, [R | t]$, 我们可以定义投影矩阵 $P$:
$
  P := K [R | t] = mat(
    delim: "[",
    p_11, p_12, p_13, p_14;
...
$
考虑到 $K$ 有 $4$ 个自由度, $[R | t]$ 有 $6$ 个自由度, 因此投影矩阵 $P$ 总共有 $10$ 个自由度. 如果要考虑 $K$ 中有倾斜因子 $s$ (也就是说两个轴不正交), 那么 $K$ 会变成
$
  K = mat(
    delim: "[",
    f_x, s, o_x;
...
$
这时候 $K$ 就有 $5$ 个自由度, 投影矩阵 $P$ 则有 $11$ 个自由度.
=== 基于三维标定物的标定 (直接线性变换)
假设我们知道一系列三维点 $bold(M)_i = (X_i, Y_i, Z_i, 1)^T$ 在世界坐标系中的位置, 以及它们在图像平面上的对应像素坐标 $bold(m)_i = (u_i, v_i, 1)^T$. 我们可以通过这些已知点来估计投影矩阵 $P$.

对于每一对对应点 $(bold(M)_i, bold(m)_i)$, 我们有以下投影关系:
$
  s_i bold(m)_i = P bold(M)_i ==> bold(m)_i times (P bold(M)_i) = 0
$
这里每一对点提供了 $2$ 个线性方程 (因为齐次坐标下有比例不变性). 考虑到投影矩阵 $P$ 有 $11$ 个自由度, 我们至少需要 $6$ 对对应点来构建一个线性方程组, 以唯一确定 $P$.

=== 基于二维标定物的标定
令相机外参矩阵 $[R | t] = [R_1 R_2 R_3 t]$. 在二维标定中, 我们假设标定物 (如棋盘格) 位于世界坐标系的 $Z = 0$ 平面上, 此时我们的投影方程变成
$
  m = lambda K [R_1 R_2 R_3 t] vec(X, Y, 0, 1) = lambda K [R_1 R_2 t] vec(X, Y, 1)
$
我们令
$
  H := lambda K [R_1 R_2 t] = [H_1 H_2 H_3]
$
为一个 $3 times 3$ 的单应矩阵 (Homography Matrix), 它描述了两个平面之间的射影变换关系. 通过这个单应矩阵, 我们可以将二维平面上的点映射到图像平面上.

考虑到二维标定物通常是已知相对位置的网格点, 我们可以通过检测图像中的这些网格点来获取它们在图像平面上的像素坐标. 和三维的情况类似, 每一对对应点 $(bold(M)_i, bold(m)_i)$ 提供 $2$ 个线性方程. 由于单应矩阵 $H$ 有 $8$ 个自由度 (9 个元素减去一个比例因子), 我们至少需要 $4$ 对对应点来唯一确定 $H$.

二维标定求解的是相机的内参 (因为不同视角下的外参会变化). 每一个视角的 $H$ 可以为解出 $K$ 提供两个约束, 因此至少需要 $3$ 个不同视角的标定图像来唯一确定相机的内参矩阵 $K$.

== 核心思想: Structure from Motion (SfM)
视觉重建的核心思想是通过多张图像中的特征点匹配, 来同时估计相机的运动 (姿态) 和场景的三维结构. 这个过程通常被称为 *Structure from Motion (SfM)*.

一个典型的增量式 SfM 流程如下:
1. *特征检测与匹配*: 在每张图像中检测特征点 (如 SIFT, ORB), 并在图像之间进行特征匹配, 以建立对应关系.
2. *初始相机姿态估计*: 选择两张图像, 使用基础矩阵估计方法 (如 RANSAC + 八点法) 来计算相机的相对姿态 (旋转和平移).
3. *三维点重建*: 通过三角测量方法, 使用匹配的特征点和相机姿态来计算三维点的坐标.
4. *增量式添加图像*: 对于每一张新的图像, 使用 PnP 算法来估计相机的姿态, 并通过三角测量来更新三维点云.
5. *全局优化 (Bundle Adjustment)*: 使用非线性优化方法, 同时优化所有相机姿态和三维点的位置, 以最小化重投影误差.

= Kalman 滤波
假设我们对一个物理过程, 有一个自己的预测模型, 以及一个可以实时观测的传感器. 考虑到预测和观察都会有误差, 我们希望通过某种方法, 最优地结合预测和观测结果, 得到对系统状态的最佳估计. Kalman 滤波器正是为此设计的.

== 观测模型
状态转移方程
$
  bold(x)_k = A_k bold(x)_(k-1) + B_k bold(u)_k + bold(w)_k
$
这里 $x_k$ 是系统在时间 $k$ 的状态向量, $A_k$ 是状态转移矩阵, 描述了系统从时间 $k-1$ 到 $k$ 的演化规律. $u_k$ 是控制输入 (如果有的话), $B_k$ 是控制输入矩阵, $w_k$ 是过程噪声, 假设服从均值为零的高斯分布, 协方差为 $Q_k$.

观测方程
$
  bold(y)_k = H_k bold(x)_k + bold(v)_k
$
这里 $y_k$ 是在时间 $k$ 的观测向量, $H_k$ 是观测矩阵, 描述了如何从状态空间映射到观测空间. $v_k$ 是观测噪声, 也假设服从均值为零的高斯分布, 协方差为 $R_k$.

== 观测阶段
状态预测
$
  hat(bold(x))_k^(-) = A_k hat(bold(x))_(k-1) + B_k bold(u)_k
$
...
$
这个增益矩阵决定了我们在更新状态时, 预测和观测的权重分配.

更新状态估计
$
  hat(bold(x))_k = hat(bold(x))_k^(-) + K_k (bold(y)_k - H_k hat(bold(x))_k^(-))
$
...
所以结果为
$
  R = 1 times -3 + 2 times -2 + 3 times -1 \ +
  4 times 0 + 5 times 0 + 6 times 0 \ +
  7 times 1 + 8 times 2 + 9 times 1 = 22
$

*注*: 如果是 "相关" 操作, 则不需要翻转卷积核.

== 形态学运算


== 中值滤波
对序列 $X = [10, 2, 3, 200, 5, 8, 9]$ 进行窗口大小为 $3$ 的中值滤波。

假设边界是补零的, 则

位置 1 (10): 窗口 $[0, 10, 2] -> [0, 2, 10]$, 中值为 $2$. \
位置 2 (2)：窗口 $[10, 2, 3] -> [2, 3, 10]$, 中值为 $3$. \
位置 3 (3)：窗口 $[2, 3, 200] -> [2, 3, 200]$, 中值为 $3$. \
位置 4 (200)：窗口 $[3, 200, 5] -> [3, 5, 200]$, 中值为 $5$. \
位置 5 (5)：窗口 $[200, 5, 8] -> [5, 8, 200]$, 中值为 $8$. \
位置 6 (8)：窗口 $[5, 8, 9] -> [5, 8, 9]$, 中值为 $8$. \
位置 7 (9)：窗口 $[8, 9, 0] -> [0, 8, 9]$, 中值为 $8$. \

最终结果为 $[2, 3, 3, 5, 8, 8, 8]$.

*注*: 边界也可以通过复制边缘值来处理, 结果会有所不同.

== 直方图均衡化
已知一副 3-bit 图像 ($L=8$), 总像素 $N=100$. 各灰度级像素数如下:

$n_0=10, n_1=20, n_2=25, n_3=20, n_4=15, n_5=5, n_6=5, n_7=0$.

要求计算原始灰度级 $r_2$ 映射后的新灰度值 $s_2$

$
  "CDF"(r_2) = sum_(i = 0)^2 P(r_i) = (n_0 + n_1 + n_2) / N = (10 + 20 + 25) / 100 = 0.55
$

所以
$
  s_2 = (L - 1) times "CDF"(r_2) = 7 times 0.55 = 3.85 approx 4
$
注意最后要四舍五入.
...
$
利用积分图公式, 计算原图中左上角为 $(2,2)$, 右下角为 $(4,4)$ 的矩形区域 (包含边界) 的像素总和.

$
  sum = I I(4,4) - I I(1,4) - I I(4,1) + I I(1,1) \ =
  110 - 20 - 20 + 5 = 75
$

== Harris 角点响应分析
在某点处, 图像梯度协方差矩阵 (结构矩阵) 为
$
  M = mat(
    delim: "[",
    25, 10;
...
$
设 $k=0.05$, 计算 $R$ 值, 并根据 $R$ 的正负性判断该点最可能的类别 (角点/边缘/平坦).

$
  R = det(M) - k times (tr(M))^2 = 215.95
$

== 算术编码
已知符号 ${x, y}$, 概率为 $P(x)=0.6, P(y)=0.4$. 若收到的编码值 (Tag) 为 $0.72$. 假设编码字符串长度为 2, 请通过计算逆推原始字符串是什么?
...
1. 第一个符号:
  ...
  由于 $0.72$ 落在 $[0.6, 1.0)$, 第一个符号是 $y$. 更新区间为 $[0.6, 1.0)$.

2. 第二个符号:
  - $x$: $[0.6, 0.84)$
  - $y$: $[0.84, 1.0)$

  由于 $0.72$ 落在 $[0.6, 0.84)$, 第二个符号是 $x$. 更新区间为 $[0.6, 0.84)$.
...
$
一个空间点在世界坐标系下的齐次坐标为 $P_w = [1, 1, 10, 1]^T$. 假设相机坐标系与世界坐标系重合 (即 $R=I, t=0$). 要求: 计算该点在图像上的像素坐标 $(u, v)$.

投影公式为
$
  s bold(m) = K [R | t] bold(M)_w = K I P_w = K P_w
$
代入数值计算:
$
  s mat(
...
所以
$
  u = 4000 / 10 = 400, quad v = 3200 / 10 = 320
$

== 双目视差与单位换算
两台相机平行放置, 焦距均为 $12$ mm, 基线 $10$ cm. 传感器像元尺寸 (Pixel Size) 为 $10$ $mu$ m/pixel. 若左图和右图中对应点的像素坐标差为 $50$ pixels, 计算该点距离相机的实际深度 (单位: 米).

首先计算图像上 $50$ 个像素对应的实际物理长度:
$
  d = 50 times 10 mu "m" = 500 mu "m" = 0.5"mm"
$
然后利用双目深度基本公式
$
  Z = (f T_x) / d = (12 times 100) / 0.5 = 2400 "mm" = 2.4 "m"
$
== 消隐点与无穷远点
在射影平面上, 空间中两组相互平行的直线, 其方向向量都是为 $bold(d) = (X, Y, Z)^T$.

写出这两条平行线在图像平面上交点 (消隐点) 的齐次坐标表示. 解释为什么在仿射变换下, 消隐点仍然在无穷远处, 而在射影变换下会变成有限坐标.

空间直线方程可表示为 $P(lambda) = P_0 + lambda bold(d)$, 其中 $P_0$ 是直线上的一个点, $lambda$ 是参数. 当 $lambda$ 趋近于无穷大时:
$
  bold(v) = lim_(lambda -> oo) K [R | t] vec(P_0 + lambda bold(d), 1) = lim_(lambda -> oo) K [R | t] vec(P_0 / lambda + bold(d), 1 / lambda) \ = K [R | t] vec(bold(d), 0) = K R bold(d)
$
可以看出来 $bold(v)$ 和 $P_0$ 无关, 因此所有平行线的投影都汇聚于同一个点 $bold(v)$, 这就是消隐点.




== 卡尔曼滤波
设状态变量为一维位置 $x$. 已知 $k-1$ 时刻状态估计 $hat(x)_(k-1)=5$, 其不确定性 $P_(k-1)=1$. 系统预测模型为 $x_k = x_(k-1) + u_k$ ($u_k=2$ 为控制增量) , 预测噪声 $Q=0.5$. 当前观测到位置 $z_k = 8$, 观测噪声 $R=0.5$.

手算预测步骤和更新步骤, 得出 $k$ 时刻的最优估计 $hat(x)_k$

1. 状态预测
$
  hat(x)_k^(-) = hat(x)_(k-1) + u_k = 5 + 2 = 7
$
2. 误差协方差预测
$
  P_k^(-) = P_(k-1) + Q = 1 + 0.5 = 1.5
$

3. 计算卡尔曼增益
$
  K_k = P_k^(-) / (P_k^(-) + R) = 1.5 / (1.5 + 0.5) = 0.75
$

4. 更新状态估计
$
  hat(x)_k = hat(x)_k^(-) + K_k (z_k - hat(x)_k^(-)) = 7 + 0.75 (8 - 7) = 7.75
$

5. 更新误差协方差
$
  P_k = (1 - K_k) P_k^(-) = (1 - 0.75) times 1.5 = 0.375
$

== DLT
已知一个空间点在世界坐标系下的齐次坐标为 $P_w = [2, 1, 5, 1]^T$, 它在图像上的像素坐标为 $(u, v) = (400, 300)$.

请根据 DLT 算法的约束方程 $x_i times (M P_i) = 0$, 写出该点对应的两行关于矩阵 $M$ 元素的线性方程.
...
$
  s mat(
    ...
    delim: "[",
    2 m_11 + 1 m_12 + 5 m_13 + m_14;
    2 m_21 + 1 m_22 + 5 m_23 + m_24;
    2 m_31 + 1 m_32 + 5 m_33 + m_34
  ) = 0
$
我们可以用行列式计算叉积:
$
  s (u bold(hat(x)) + v bold(hat(y)) + 1 bold(hat(z))) =
  mat(
    delim: "|",
...
$
其中 $M_1, M_2, M_3$ 分别是 $M P_w$ 的三行.

== 对极线方程
已知左图像中一个像素点的齐次坐标为 $bold(x) = [200, 100, 1]^T$. 两视图间的基础矩阵为
$
  F = mat(
    delim: "[",
    0, 0.001, -0.2;
...
根据对极约束公式 $bold(x)'^T F bold(x) = 0$, 求出该点在右图像上对应的对极线方程 $l'$ (写成 $a x' + b y' + c = 0$ 的形式).

$bold(x)'^T F bold(x)$ 可以被写成直线方程 $bold(x)'^T bold(l)' = 0$, 其中
$
  bold(l)' = F bold(x) = mat(
    delim: "[",
    0, 0.001, -0.2;
...
== 本质矩阵
假设相机坐标系 2 相对于相机坐标系 1 的变换为: 先绕 $Z$ 轴顺时针旋转 90°, 然后沿 $X$ 轴平移 2 个单位.

1. 写出旋转矩阵 $R$ 和平移向量 $bold(t)$.

2. 计算平移向量的反对称矩阵 $[bold(t)]_times$.

3. 根据公式 $E = [bold(t)]_x R$, 计算本质矩阵 $E$.

1. 顺时针旋转 90° 的旋转矩阵为 ($theta = - pi / 2$)
$
  R_z = mat(
    delim: "[",
...
*注*: 一般情况下的旋转矩阵可以被表示为 $R = R_z R_y R_x$, 其中 $R_x, R_y, R_z$ 分别是绕 $X, Y, Z$ 轴的旋转矩阵.
$
  R_x (theta) = mat(
    delim: "[",
    1, 0, 0;
